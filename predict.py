...
from utils import find_optimal_threshold, cal_scores
from dataset import ECGDataset
from utils import split_data
from torch.utils.data import DataLoader
from resnet import resnet34, resnet18
from calculate_cost import test_model_inference_speed, get_model_size
import psutil
...
def plot_cm(y_trues, y_preds, normalize=True, cmap=plt.cm.Blues):
    ...
    for i, label in enumerate(classes):
        ...
        fig.tight_layout()
        plt.savefig(f'figure/figure{label}.png')
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    args.model_path = f'models/resnet18_{database}_{args.leads}_{args.seed}.pth'
   ...
   # Mengukur ukuran model
    model_size = get_model_size(args.model_path)
    print(f'Model size: {model_size / (1024 ** 2):.2f} MB')

    # Mengukur kecepatan inferensi
    inference_speed = test_model_inference_speed(test_loader, net, device)
    print(f'Average inference speed: {inference_speed * 1000:.2f} ms/sample')

    # Mengukur konsumsi memori sebelum testing
    process = psutil.Process(os.getpid())
    mem_before = process.memory_info().rss / (1024 ** 2)
    print(f'Memory consumption before testing: {mem_before:.2f} MB')

    # Start testing and measure memory during inference
    with torch.no_grad():
        start_mem = torch.cuda.memory_allocated(device)  # GPU memory before inference
        # Apply model on test data
        for _, (data, _) in enumerate(test_loader):
            data = data.to(device)
            net(data)
        end_mem = torch.cuda.max_memory_allocated(device)  # GPU memory after inference

    
    # Mengukur konsumsi memori setelah testing
    mem_after = process.memory_info().rss / (1024 ** 2)
    print(f'Memory consumption after testing: {mem_after:.2f} MB')
    print(f'Peak GPU memory consumption during inference: {(end_mem - start_mem) / (1024 ** 2):.2f} MB')
